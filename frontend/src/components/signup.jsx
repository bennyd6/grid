...
const TemplateDisplay = () => {
  ...
  const [authModalMessage, setAuthModalMessage] = useState(""); // New state for auth modal message
  ...
  const handleHostTemplate = async (templateId) => {
    ...
    if (authLoading) {
      setAuthModalMessage("Please wait while we verify your login status.");
  ...
  const closeAuthRequiredModal = () => {
    ...
    // Only reload if the user was *not* logged in (and authLoading is false)
    ...
    <div className="relative w-screen min-h-screen overflow-hidden">
      ...
      <AnimatePresence>
        {showAuthRequiredModal && (
          ...
          >
            <motion.div
              className="bg-gray-800 rounded-lg p-8 w-full max-w-md text-gray-100 shadow-2xl border border-gray-700 text-center"
              initial={{ scale: 0.8, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.8, opacity: 0 }}
            >
